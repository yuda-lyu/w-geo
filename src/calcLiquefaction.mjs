...
function isNoLique(soilClassification) {
    ...
    //C,O,P,M(除了ML與MH)視為非液化條件 (2021/02/04)
    if (r1 === 'C' || r1 === 'O' || r1 === 'P') {
        return true
    }
...
function interp(vx, vy, x) {
    ...
    //check
    if (size(vx) <= 0) {
        //console.log('invalid vx', vx)
        return 'invalid vx'
    }
    if (size(vy) <= 0) {
        //console.log('invalid vy', vy)
        return 'invalid vy'
    }
    if (size(vx) !== size(vy)) {
        //console.log('size of vx is not equal to size of vy', vx, vy)
        return 'size of vx is not equal to size of vy'
    }
    ...
    if (volumetricStrain.err) {
        ...
        if (volumetricStrain.err === 'out of x-range') {
            ...
            if (N160 > N160Max) {
                ...
                err.push(`出現外插情形 N160=${N160} > N160Max=${N160Max}，依照CSR=${CSR}並給予最小體積應變 ${vstr}%`)
                volumetricStrain = vstr
            }
            else if (N160 < N160Min) {
                let vstr = interExtraPolate(ps[0], ps[1], N160)
                err.push(`出現外插情形 N160=${N160} < N160Min=${N160Min}，依照CSR=${CSR}並依據最近2點外插體積應變 ${vstr}%`)
                volumetricStrain = vstr
            }
            else {
...
function sptVolumetricStrainIshiharaAndYoshimine(N172, FS) {
    ...
    if (volumetricStrain.err) {
        ...
        if (volumetricStrain.err === 'out of x-range') {
            ...
            if (N172 > N172Max) {
                let vstr = ps[size(ps) - 1].y //x為遞增, 故最後1點y為最小體積應變
                err.push(`出現外插情形 N172=${N172} > N172Max=${N172Max}，依照FS=${FS}並給予最小體積應變 ${vstr}%`)
                volumetricStrain = vstr
            }
            else if (N172 < N172Min) {
                let vstr = interExtraPolate(ps[0], ps[1], N172)
                err.push(`出現外插情形 N172=${N172} < N172Min=${N172Min}，依照FS=${FS}並依據最近2點外插體積應變 ${vstr}%`)
                volumetricStrain = vstr
            }
            else {
                err.push(`非預期外插情形，請洽開發者修復問題`)
...
function sptSeed({ waterLevel, soilClassification, depth, N60, FC, svp, sv, PGA, Mw }) {
    //Seed et al.(1985)
    let err = []
    let MSF = ''
    let rd = ''
    ...
    //check waterLevel
    if (!isnum(waterLevel)) {
        err.push(`waterLevel${brk(waterLevel)} 非數字，預設為0 (m)`)
        waterLevel = 0
    }
    waterLevel = cdbl(waterLevel)
    if (waterLevel < 0) {
        err.push(`waterLevel${brk(waterLevel)} < 0，預設為0 (m)`)
        waterLevel = 0
    }
    ...
    if (depth < waterLevel || isNoLique(soilClassification)) {
        CRR = '-'
        CSR = '-'
        FS = 10
        return ret()
    ...
    if (!isnum(svp)) {
        err.push(`svp${brk(svp)} 非數字`)
        return ret()
    }
    svp = cdbl(svp)
    if (svp < 0) {
        err.push(`svp${brk(svp)} < 0`)
        return ret()
    }
...
function sptHBF({ ver = '2012', waterLevel, soilClassification, depth, N60, FC, PI, svp, sv, PGA, Mw }) {
    //HBF(ver=2012,2017)
    let err = []
    let MSF = ''
    let rd = ''
...
function sptNCEER({ waterLevel, soilClassification, depth, N60, FC, svp, sv, PGA, Mw }) {
    //NCEER(1997)
    let err = []
    let MSF = ''
    let rd = ''
...
function sptNJRA({ ver = '1996', waterLevel, soilClassification, vibrationType, depth, N60, FC, PI, D50, D10, svp, sv, PGA, Mw }) {
    //NJRA(ver=1996,2017)
    let err = []
    let rd = ''
    let N72 = ''
    ...
    if (depth < waterLevel || waterLevel > 10) {
        CRR = '-'
        CSR = '-'
        FS = 10
        return ret()
    ...
    if (!(isSandByUSCS(soilClassification) || isGravelByUSCS(soilClassification))) {
        CRR = '-'
        CSR = '-'
        FS = 10
        return ret()
...
function sptTY({ waterLevel, soilClassification, depth, N60, FC, svp, sv, PGA, Mw }) {
...
function cptJuang({ waterLevel, depth, qc, fs, svp, sv, PGA, Mw }) {
    let err = []
    let CRR = ''
    let CSR = ''
    let FS = ''
...
function cptHBF({ waterLevel, depth, qc, fs, svp, sv, PGA, Mw }) {
    let err = []
    let CRR = ''
    let CSR = ''
    let FS = ''
    ...
    let MSF = (Mw / 7.5) ** (-3.3)
    let rd = (1 - 0.4113 * depth ** 0.5 + 0.04052 * depth + 0.001753 * depth ** 1.5) / (1 - 0.4177 * depth ** 0.5 + 0.05729 * depth - 0.006205 * depth ** 1.5 + 0.00121 * depth ** 2)
    let qc1N = 1.7 / (svp + 0.7) * qc
    console.log('需確認公式內svp是否為kg/cm2, 否則需要轉kPa, 不過170=>1.7與70=>0.7就代表除r = 98.134256')
    let Rf = fs / qc * 100
    let Kc
    if (Rf <= 0.1) {
        Kc = 1.0
...
function cptOlsen({ waterLevel, depth, qc, fs, svp, sv, PGA, Mw }) {
    let err = []
    let CRR = ''
    let CSR = ''
    let FS = ''
...
function cptShibata({ waterLevel, depth, qc, fs, svp, sv, PGA, Mw, D50 }) {
    let err = []
    let CRR = ''
    let CSR = ''
    let FS = ''
    ...
    let qc1 = 1.7 / (svp + 0.7) * qc
    let rd = 1 - 0.015 * depth
    let TempL = 0.1 * (Mw - 1) * PGA * (sv / svp) * rd
    let C2
...
function cptNCEER({ waterLevel, depth, qc, fs, svp, sv, PGA, Mw }) {
    let err = []
    let CRR = ''
    let CSR = ''
    let FS = ''
    ...
    let rd = (1 - 0.4113 * depth ** 0.5 + 0.04052 * depth + 0.001753 * depth ** 1.5) / (1 - 0.4177 * depth ** 0.5 + 0.05729 * depth - 0.006205 * depth ** 1.5 + 0.00121 * depth ** 2)
    let Q = (qc - sv) / svp
    console.log('待確認qc,sv,svp單位皆需為kg/cm2')
    let F = (fs / (qc - sv)) * 100
    console.log('待確認fs,qc,sv單位皆需為kg/cm2')
    let qc1N
    ...
    if (Ic1 <= 2.6) {
        ...
        qc1N = (qc / Pa2) * (Pa / svp) ** 0.5
        console.log('待確認qc,Pa,Pa2,svp的單位')
        let Ic2 = Math.sqrt((3.47 - Math.log10(qc1N)) ** 2 + (Math.log10(F) + 1.22) ** 2)
        if (Ic2 <= 2.6) {
            Ic3 = Ic2
        }
        else {
            qc1N = (qc / Pa2) * (Pa / svp) ** 0.75
            console.log('待確認qc,Pa,Pa2,svp的單位')
...
function vsHBF({ waterLevel, depth, Vs, FC, svp, sv, PGA, Mw }) {
    let err = []
    let CRR = ''
    let CSR = ''
    let FS = ''
    ...
    let MSF = (Mw / 7.5) ** (-3.3)
    let rd = (1 - 0.4113 * depth ** 0.5 + 0.04052 * depth + 0.001753 * depth ** 1.5) / (1 - 0.4177 * depth ** 0.5 + 0.05729 * depth - 0.006205 * depth ** 1.5 + 0.00121 * depth ** 2)
    let Vs1 = 1 / Math.sqrt(svp) * Vs
    console.log('待確認此處svp,Vs的單位')
    let Kv
    if (FC <= 10) {
        Kv = 1.0
    }
...
function vsAndrus({ waterLevel, depth, Vs, FC, svp, sv, PGA, Mw }) {
    let err = []
    let CRR = ''
    let CSR = ''
    let FS = ''
...
function vsNCEER({ waterLevel, depth, Vs, FC, svp, sv, PGA, Mw }) {
    let err = []
    let CRR = ''
    let CSR = ''
    let FS = ''
...
function calcLiquefaction(kind = 'auto', rows) {
    // console.log('calcLiquefaction', kind, rows)
    ...
    function addDepthAndPorousParams(waterLevel, rows) {
    ...
    function liqParams(kind, waterLevel, rows) {

        //waterLevel
        waterLevel = cdbl(waterLevel)

        //addDepthAndPorousParams
        rows = addDepthAndPorousParams(waterLevel, rows)

        if (kind === 'SPT') {

            //calcVerticalStress, sv:土層中點深度之垂直總應力(kN/m2), svp:土層中點深度之垂直有效應力(kN/m2)
            rows = calcVerticalStress(waterLevel, rows)
    ...
    function liqFS(kind, row) {
        ...
        //各種液化方法
        each(methodLique[kind], (func, method) => {
    ...
    //waterLevel, 不給地下水位就預設至地表
    let waterLevel = get(rows, '0.waterLevel', 0)
    ...
    //liqParams, 自動計算參數
    rows = liqParams(kind, waterLevel, rows)
...
let calcLiquefaction = {
    liq: methodLique,
    sett: methodSettlement,
    calc: calcLiquefaction,
}
