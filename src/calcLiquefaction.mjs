...
//limFSNoliqForBasic, limFSNoliqForSpec, limFSMax
let limFSNoliqForBasic = 10 //基本非液化條件之安全係數
let limFSNoliqForSpec = 3 //指定非液化條件之安全係數
...
### 1.1 國震
- 「0206地震花蓮港震陷與土壤液化勘查」國震報告
- https://www.ncree.narl.org.tw/assets/file/P65--Corrected_Proof--156--0206%E5%9C%B0%E9%9C%87%E8%8A%B1%E8%93%AE%E6%B8%AF%E6%B6%B2%E5%8C%96%E9%9C%87%E9%99%B7%E8%88%87%E5%9C%9F%E5%A3%A4%E6%B6%B2%E5%8C%96%E5%8B%98%E6%9F%A5--%E8%A8%B1%E5%B0%9A%E9%80%B8%E7%AD%89.pdf
> 遇到完全無液化可能之地層其FS直接取3

### 1.2 李咸亨
- 李咸亨審查意見
> 黏土不能用簡易液化評估法，故FS不顯示(繪製點超過圖上限之外)

## 二、系統處理原則

### 2.1 直接視為非液化條件不須計算FS
1. FS=10 : 土層之土壤分類為黏土或非指定粉土(土壤分類開頭C,O,P,M(但除了ML與MH))視為非液化
1. FS=10 : 土層之中點深度位於地下水以上視為非液化
1. FS=10 : 土層之中點深度位於20m以下視為非液化
1. FS=3  : 土層之SPTN值>=50視為非液化
1. FS=3  : HBF法特有: PI值>7視為非液化
1. FS=3  : HBF法特有: N160cs>=39視為非液化
1. FS=3  : NCEER法特有: N160cs>=30視為非液化
1. FS=3  : JRA法特有: 地下水低於地表下10m以下視為非液化
1. FS=3  : JRA法特有: 細料含量>35%且PI值>=15視為非液化
1. FS=3  : JRA法特有: D50>10mm或D10>1mm視為非液化

### 2.2 可進行液化分析但無法計算有效FS
1. FS=3 : 垂直總應力sv<=0
1. FS=3 : 地震矩規模Mw<=1
1. FS=3 : 最大地表加速度PGA<=0
1. FS=3 : 應力折減係數rd<=0
...
function sptSeed({ noLiqueMode = 'new', waterLevelDesign, soilClassification, depth, N60, FC, sv, svpUsual, svpDesign, PGA, Mw }) {
    ...
    if (isNumber(CSR) && CSR === 0) {
        if (sv === 0) {
            stateFS.push(`sv${brk(sv)}=0，FS強制改為${limFSMax}`)
        }
        if (PGA === 0) {
            stateFS.push(`PGA${brk(PGA)}=0，FS強制改為${limFSMax}`)
        }
        if (rrd === 0) {
            stateFS.push(`rrd${brk(rrd)}=0，FS強制改為${limFSMax}`)
        }
        FS = limFSMax
    }
    if (isNumber(FS) && FS > limFSMax) { //針對液化土(砂土與粉土但非ML與非MH)要繪製FS至圖內, 圖內FS最大值為3, 故轉設定上限為3
        stateFS.push(`FS${brk(FS)}>${limFSMax}，強制改為${limFSMax}`)
        FS = limFSMax
    }
...
function sptTY({ noLiqueMode = 'new', waterLevelDesign, soilClassification, depth, a, n, Cr, Cs, N60, FC, sv, svpUsual, svpDesign, PGA, Mw }) {
    ...
    if (isNumber(CSR) && CSR === 0) {
        ...
        if (Mw <= 1) {
            stateFS.push(`Mw${brk(Mw)}<=1，FS強制改為${limFSMax}`)
        }
        if (PGA === 0) {
            stateFS.push(`PGA${brk(PGA)}=0，FS強制改為${limFSMax}`)
        }
